  ...
  const startAnimation = () => {
    if (route && route.length > 0) {
      ...
      const animateShip = () => {
        ...
        const newTraveledPath = traveledPath.slice(0, step);
        newTraveledPath.push(
          route[step],
          [
            route[step][0] + (route[step + 1][0] - route[step][0]) * progress,
            route[step][1] + (route[step + 1][1] - route[step][1]) * progress
          ]
        );
        ...
        animationRef.current = requestAnimationFrame(animateShip);
      }
      animationRef.current = requestAnimationFrame(animateShip);
    }
  }
